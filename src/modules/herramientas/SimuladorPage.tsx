...
function calcBonuses(cfg: SimConfig): BonoBreakdown {
  ...
  // ── Bono Match (by rank on UNINIVEL earnings from SPONSOR TREE) ──
  // For each sponsor tree level (1-5), get unilevel bonus from unilevel table
  // Apply percentage based on user's rank at that sponsor level
  const matchPcts = MATCH_BONUS[rango]
  const match: BonoBreakdown['match'] = []
  let matchTotal = 0

  // Match applies to unilevel bonus generated BY people at each sponsor tree level
  for (let l = 1; l <= Math.min(levels, 5); l++) {
    const ul = unilevel[l - 1]
    if (ul && ul.bono > 0) {
      const pct = matchPcts[l - 1] || 0
      if (pct > 0) {
        // Match bonus = unilevel earnings from people at this sponsor level × rank percentage
        const bono = ul.bono * pct
        match.push({
          level: l,
          personas: ul.personas,
          unilevelEarnings: ul.bono, // Total unilevel bonus at this sponsor tree level
          bono,
          pct: pct * 100,
        })
        matchTotal += bono
