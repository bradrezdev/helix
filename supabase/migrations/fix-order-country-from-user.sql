-- Fix: place_order_with_membership RPC should set order.country from user profile
-- Root cause: orders.country defaults to 'USD' but should match buyer's country
-- Fix: SELECT country from users table and set it in the INSERT
